...
export default function HomePage() {
  ...
  <div className="absolute inset-0 bg-[#0f2044]/60 z-0" />
        ...
        <Link to="/publications" className="border border-[#39e07a]/50 text-white font-semibold px-7 py-3 rounded-full text-sm hover:border-[#39e07a] hover:bg-[#39e07a]/10 transition-all">
        View Publications
        </Link>
        </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-start">
            ...
            <div className="grid grid-cols-2 gap-3">
              {researchAreas.slice(0, 6).map((area) => {
                ...
                return (
                  <div key={area.id} className="bg-white dark:bg-[#0d1a2e] rounded-2xl p-4 border border-gray-100 dark:border-white/10 shadow-sm hover:shadow-md transition-all group">
                    ...
                    <div className="text-sm font-semibold text-[#0f2044] dark:text-white">{area.title}</div>
                  </div>
                )
              })}
            </div>
          ...
          <div className="bg-white dark:bg-[#0d1a2e] rounded-3xl border border-gray-100 dark:border-white/10 shadow-sm p-8 flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-5 text-sm text-[#6b7a94] dark:text-white/50">
                <a href={`tel:${director.phone}`} className="flex items-center gap-1.5 hover:text-[#0f2044] dark:hover:text-white transition-colors">📞 {director.phone}</a>
                <a href={`mailto:${director.email}`} className="flex items-center gap-1.5 hover:text-[#0f2044] dark:hover:text-white transition-colors">✉️ {director.email}</a>
              </div>
            </div>
          </div>
        </div>
              ...
              return (
                <div key={item.id} className="bg-white dark:bg-[#0d1a2e] rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm p-6 flex gap-5">
                  ...
                  <div className="flex-1 border-l border-gray-100 dark:border-white/10 pl-5">
                    <h3 className="font-semibold text-[#0f2044] dark:text-white text-sm mb-1.5">{item.title}</h3>
                    <p className="text-[#6b7a94] dark:text-white/40 text-xs leading-relaxed mb-3">{item.summary}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {item.tags?.map((tag) => (
                        <span key={tag} className="text-xs px-2 py-0.5 rounded-full bg-[#0f2044]/6 dark:bg-white/8 text-[#0f2044]/60 dark:text-white/50">{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
      ...
      <section className="py-20 bg-[#0f2044]">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <a href="mailto:prism-lab@iitg.ac.in" className="border border-[#39e07a]/50 text-white font-semibold px-6 py-2.5 rounded-full text-sm hover:border-[#39e07a] transition-all">Get In Touch</a>
          </div>
        </div>
      </section>
    </div>
