...
import { ChevronDown, Sun, Moon, Phone, Mail, MapPin, Menu, X } from "lucide-react";
...
export function Navbar({ dark, setDark }) {
  ...
  return (
    <nav style={{
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 20px", height: 72, display: "flex", alignItems: "center", justifyContent: "space-between" }}>

        {/* Logo */}
        <div style={{ display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
          <Link to="/" style={{ textDecoration: "none", display: "flex", alignItems: "center" }}>
            <img src="/images/prism-logo.jpg" alt="PRISM Lab" style={{ height: 56, objectFit: "contain" }}
              onError={e => e.target.style.display = "none"} />
          </Link>
          <div style={{ width: 1, height: 44, background: "var(--border)" }} />
          <a href="https://www.iitg.ac.in" target="_blank" rel="noreferrer">
            <img src="/images/iitg-logo.jpg" alt="IIT Guwahati" style={{ height: 56, width: 56, borderRadius: "50%", objectFit: "cover" }}
              onError={e => e.target.style.display = "none"} />
          </a>
        </div>
        ...
        <div className="desktop-nav" style={{ display: "flex", alignItems: "center", gap: 26 }}>
          ...
          <div style={{ position: "relative" }} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            <button onClick={() => setPeopleOpen(o => !o)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, fontSize: 15, fontWeight: isPeopleActive ? 600 : 400, color: isPeopleActive ? "var(--text-primary)" : "var(--text-secondary)", fontFamily: "Inter, sans-serif", padding: "8px 0" }}>
              People <ChevronDown size={14} style={{ transform: peopleOpen ? "rotate(180deg)" : "none", transition: "transform 0.2s" }} />
            </button>
            <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, height: 8, background: "transparent" }} />
            {peopleOpen && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, background: dark ? "#0d1a30" : "#ffffff", border: "1px solid var(--border)", borderRadius: 10, padding: "8px 0", minWidth: 170, zIndex: 500, boxShadow: "0 12px 32px rgba(0,0,0,0.35)" }}>
                {[{ label: "Lab Director", path: "/people/lab-director" }, { label: "Students", path: "/people/students" }, { label: "Alumni", path: "/people/alumni" }].map(item => (
                  <Link key={item.path} to={item.path} onClick={() => setPeopleOpen(false)}
                    style={{ display: "block", padding: "10px 20px", fontSize: 14, color: isActive(item.path) ? "var(--accent)" : "var(--text-secondary)", textDecoration: "none", fontWeight: isActive(item.path) ? 600 : 400 }}>
                    {item.label}
                  </Link>
                ))}
              </div>
          ...
          <button onClick={() => setDark(!dark)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-secondary)", display: "flex", alignItems: "center", padding: 4 }}>
        ...
        <div className="mobile-nav-btn" style={{ display: "none", alignItems: "center", gap: 12 }}>
          <button onClick={() => setDark(!dark)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-secondary)", display: "flex", alignItems: "center" }}>
            {dark ? <Sun size={18} /> : <Moon size={18} />}
          </button>
          <button onClick={() => setMobileOpen(o => !o)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-primary)", display: "flex", alignItems: "center", padding: 4 }}>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div style={{ background: "var(--navbar-bg)", borderTop: "1px solid var(--border-light)", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 4 }}>
          {[{ label: "Home", path: "/" }, { label: "Lab Director", path: "/people/lab-director" }, { label: "Students", path: "/people/students" }, { label: "Alumni", path: "/people/alumni" }, { label: "Infrastructure", path: "/infrastructure" }, { label: "Research", path: "/research" }, { label: "Publications", path: "/publications" }, { label: "News", path: "/news" }, { label: "Gallery", path: "/gallery" }].map(item => (
            <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)}
              style={{ display: "block", padding: "12px 8px", fontSize: 16, color: isActive(item.path) ? "var(--accent)" : "var(--text-secondary)", textDecoration: "none", fontWeight: isActive(item.path) ? 600 : 400, borderBottom: "1px solid var(--border-light)" }}>
              {item.label}
            </Link>
          ))}
        </div>
